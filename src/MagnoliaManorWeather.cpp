...
#line 1 "d:/OneDrive/Documents/Particle/magnolia-manor-weather-station/src/MagnoliaManorWeather.ino"
/******************************************************************************
...
float calibrateTemp(String command);
void printInfo();
void getSoilTemp();
...
#line 121 "d:/OneDrive/Documents/Particle/magnolia-manor-weather-station/src/MagnoliaManorWeather.ino"
...
#define DEFAULT_TEMPERATURE_OFFSET_F 8.5 // offset due to heat generated by photon board inside weather station housing deg F
...
double temperature_offset_f = DEFAULT_TEMPERATURE_OFFSET_F;
double temperature_offset_c = (temperature_offset_f / 1.8);
...
void setup()
{
    ...
    Particle.function("calibrateTemp", calibrateTemp);
    ...
    Particle.variable("temperature_offset_f",temperature_offset_f);//calibration offset for temperature measurements
    Particle.variable("temperature_offset_c",temperature_offset_c);//calibration offset for temperature measurements
...
void loop()
{
  ...
  if(millis() - lastSecond >= 1000)
  {
    ...
    if(((long) millis()) - lastPublish > delayPublish) //Publishes every 2000 milliseconds, or 2 seconds
    {
...
#ifdef ALTITUDE_MODE
      success = Particle.publish("MMWEATHER", String::format("{\"humidity\":%f,\"tempF\":%f,\"altf\":%f,\"baroTempF\":%f,\"SoilTempF\":%f,\"SoilMoisture\":%d,\"LuminousFlux\":%f,\"AirTVOC\":%d,\"airCO2\":%d, \"winddir\":%d,\"windspeedmph\":%f,\"windgustmph\":%f,\"windgustdir\":%d,\"windspdmph_avg2m\":%f,\"winddir_avg2m\":%d,\"windgustmph_10m\":%f,\"windgustdir_10m\":%d,\"rainin\":%f,\"dailyrainin\":%f}",humidity,tempf,altf,baroTemp,soiltempf,soilMoisture,LuminousFlux > 65534 ? 65534 : LuminousFlux,airTVOC, airCO2, winddir,windspeedmph,windgustmph,windgustdir,windspdmph_avg2m,winddir_avg2m,windgustmph_10m,windgustdir_10m,rainin,dailyrainin), 60, PRIVATE);
#else
      success = Particle.publish("MMWEATHER", String::format("{\"humidity\":%f,\"tempF\":%f,\"pressurehPa\":%f,\"pressureHg\":%f,\"baroTempF\":%f,\"SoilTempF\":%f,\"SoilMoisture\":%d,\"LuminousFlux\":%f,\"AirTVOC\":%d,\"airCO2\":%d, \"winddir\":%d,\"windspeedmph\":%f,\"windgustmph\":%f,\"windgustdir\":%d,\"windspdmph_avg2m\":%f,\"winddir_avg2m\":%d,\"windgustmph_10m\":%f,\"windgustdir_10m\":%d,\"rainin\":%f,\"dailyrainin\":%f}",humidity,tempf,(pascals/100),((pascals/100) * 0.0295300),baroTemp,soiltempf,soilMoisture,LuminousFlux > 65534 ? 65534 : LuminousFlux,airTVOC, airCO2, winddir,windspeedmph,windgustmph,windgustdir,windspdmph_avg2m,winddir_avg2m,windgustmph_10m,windgustdir_10m,rainin,dailyrainin), 60, PRIVATE);
...
// Allows changing temperature calibration
float calibrateTemp(String command) {
    temperature_offset_f = atof(command);
    temperature_offset_c = (temperature_offset_f / 1.8);
    return temperature_offset_f;
}
...
//    windgustdir = 0;
    ...
    if(++seconds > 59)
    {
      ...
      windgustmph = currentSpeed; // measure wind gusts over a one minute period
      windgustdir = currentDirection;
    }
